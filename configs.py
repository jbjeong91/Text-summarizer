...
tf.app.flags.DEFINE_string('cmd', 'train', 'cmd')  # train or test
...
tf.app.flags.DEFINE_integer('max_sequence_length', 50, 'max sequence length')  # 시퀀스 길이
...
tf.app.flags.DEFINE_integer('layer_size', 2, 'layer size')
tf.app.flags.DEFINE_string('data_path', './data_in/news_summary_more.csv', 'data path')  # 아마존 리뷰 데이터
tf.app.flags.DEFINE_string('vocabulary_path', './data_out/vocabularyData.voc', 'vocabulary path')  # 사전 위치
tf.app.flags.DEFINE_string('check_point_path', './data_out/check_point_news', 'check point path')  # 체크 포인트 위치
